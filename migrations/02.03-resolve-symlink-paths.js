...
module.exports = {
  version:     2.03,
  description: 'Resolve symlink paths to real paths in roots and files',
  migrate(db) {
      ...
      try {
        ...
        if (realPath !== root.path) {
          if (rootExists.get(realPath)) {
            // Real path already exists as a root - delete the symlink entry
    ...
    const countRows = db.prepare('SELECT COUNT(*) as count FROM files').get();
    const totalFiles = countRows.count;
    const selectFiles = db.prepare('SELECT path, mtime_ms, mtime FROM files');
    ...
    const allFiles = selectFiles.all();
